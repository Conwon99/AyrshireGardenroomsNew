...
interface DeckingFeaturesProps {
  location?: { name: string; regionLabel?: string };
}

export const DeckingFeatures = ({ location }: DeckingFeaturesProps) => {
  const features = [
    {
      icon: (
      ...
      description: "We use premium timber and composite decking materials that are weather-resistant and built to last. All materials are chosen for durability and low maintenance."
    },
    {
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      description: "Professional deck installation by our experienced team. Most decks are completed within 2-5 days, with minimal disruption to your garden."
    },
    {
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      title: "Custom Design",
      description: "Every deck is designed to fit your space perfectly. From simple platforms to multi-level decks with railings and steps, we create the perfect solution for your garden."
    },
    {
      icon: (
        <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="box-border caret-transparent gap-x-[30px] flex flex-col justify-between gap-y-[30px] md:gap-x-[50px] md:gap-y-[50px]">
            <div className="box-border caret-transparent">
              <h2 className="text-gray-900 text-3xl font-bold box-border caret-transparent leading-[38px] mb-2.5 sm:text-4xl sm:leading-[43.2px] md:text-[56px] md:leading-[67.2px]">
                {location ? `Why Choose Our Decking in ${location.name}?` : "Why Choose Our Decking?"}
              </h2>
